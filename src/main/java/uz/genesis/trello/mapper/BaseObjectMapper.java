...
import uz.genesis.trello.config.handler.CustomAnnotationHandler;
...
public class BaseObjectMapper {
    ...
    protected <C> C toDto(Object[] resultSet, String mapperMethodName) throws Exception {
        ...
        for (int i = 1; i < resultSetList.size(); i++) {
            ...
            } else if (object.getClass().getSimpleName().equals(field.getType().getSimpleName().replace("Dto", ""))) {
                field.set(item, toDto(object, generateMapperBeanName(object), "toDto"));
            }
        }
    ...
    public <C> List<C> toDtoList(List<Object[]> resultList, String mapperMethodName) throws Exception {
        ...
        for (int i = 0; i < length; i++) {
            dtoList.add(toDto(resultList.get(i), mapperMethodName));
        }
        return dtoList;
    }
