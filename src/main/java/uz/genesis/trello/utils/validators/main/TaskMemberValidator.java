...
public class TaskMemberValidator extends BaseCrudValidator<TaskMember, TaskMemberCreateDto, CrudDto> {
    ...
    public void baseValidation(TaskMember domain, boolean idRequired) {
        if (utils.isEmpty(domain)) {
            throw new RequestObjectNullPointerException(repository.getErrorMessage(ErrorCodes.OBJECT_IS_NULL, utils.toErrorParams(TaskMember.class)), "TaskMember");
            ...
            throw new ValidationException(repository.getErrorMessage(ErrorCodes.OBJECT_GIVEN_FIELD_REQUIRED, utils.toErrorParams("taskId", TaskMember.class)), "taskId");
        } else if (utils.isEmpty(domain.getEmployee())) {
            throw new ValidationException(repository.getErrorMessage(ErrorCodes.OBJECT_GIVEN_FIELD_REQUIRED, utils.toErrorParams("employee", TaskMember.class)), "employee");
        }
    }
}
